<?php
/**
 * StellarPush 触发入口：供服务器计划任务 curl 调用，或插件内部 fire-and-forget 调用。
 *
 * 用法：
 *   curl "https://lioip.cn/usr/plugins/StellarPush/push.php?key=你的密钥&mode=incremental"
 *
...
$internal = \TypechoPlugin\StellarPush\Plugin::state('internal_key', '');
...
if ($mode === 'single') {
    ...
    $result = \TypechoPlugin\StellarPush\Plugin::pushSingle($cid);
} elseif ($mode === 'dry') {
    $result = \TypechoPlugin\StellarPush\Plugin::push('incremental', true);
} else {
    $result = \TypechoPlugin\StellarPush\Plugin::push($mode, false);
}
